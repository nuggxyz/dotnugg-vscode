...
class Decorator {
    ...
    public static addColorsToData(file: Compiler) {
        try {
            ...
            for (let i = 0; i < file.results.attributes.length; i++) {
                const attr = file.results.attributes[i].value;
                ...
                x.data.value.matrix.forEach((r, yindex) => {
                    r.value.forEach((c, xindex) => {
                        try {
                            ...
                            if (
                                feature.validRadii.value.r.value > 0 &&
                                ...
                                feature.validRadii.value.l.value > 0 &&
                                yindex + 1 === x.anchor.value.y.value &&
                                xindex + 1 < x.anchor.value.x.value &&
                                ...
                                feature.validRadii.value.d.value > 0 &&
                                xindex + 1 === x.anchor.value.x.value &&
                                yindex + 1 > x.anchor.value.y.value &&
                                ...
                                feature.validRadii.value.u.value > 0 &&
                                xindex + 1 === x.anchor.value.x.value &&
                                yindex + 1 < x.anchor.value.y.value &&
                            ...
                            vscode.window.activeTextEditor
                                .edit((builder) => {
                                    if (feature.validRadii.value.r.value === 0 && x.radii.value.r.value !== 0) {
                                        builder.replace(x.radii.value.r.token.range, '0');
                                    }
                                    if (feature.validRadii.value.l.value === 0 && x.radii.value.l.value !== 0) {
                                        builder.replace(x.radii.value.l.token.range, '0');
                                    }
                                    if (feature.validRadii.value.d.value === 0 && x.radii.value.d.value !== 0) {
                                        builder.replace(x.radii.value.d.token.range, '0');
                                    }
                                    if (feature.validRadii.value.u.value === 0 && x.radii.value.u.value !== 0) {
                                        builder.replace(x.radii.value.u.token.range, '0');
                                    }
                                    if (feature.validRadii.value.r.value === 0 && x.expanders.value.r.value !== 0) {
                                        builder.replace(x.expanders.value.r.token.range, '0');
                                    }
                                    if (feature.validRadii.value.l.value === 0 && x.expanders.value.l.value !== 0) {
                                        builder.replace(x.expanders.value.l.token.range, '0');
                                    }
                                    if (feature.validRadii.value.d.value === 0 && x.expanders.value.d.value !== 0) {
                                        builder.replace(x.expanders.value.d.token.range, '0');
                                    }
                                    if (feature.validRadii.value.u.value === 0 && x.expanders.value.u.value !== 0) {
                                        builder.replace(x.expanders.value.u.token.range, '0');
                                    }
                                })
                                .then();

                            if (feature.validRadii.value.r.value === 0) {
                                fadedRanges.push(x.radii.value.r.token.range);
                                fadedRanges.push(x.expanders.value.r.token.range);
                            }
                            if (feature.validRadii.value.l.value === 0) {
                                fadedRanges.push(x.radii.value.l.token.range);
                                fadedRanges.push(x.expanders.value.l.token.range);
                            }
                            if (feature.validRadii.value.d.value === 0) {
                                fadedRanges.push(x.radii.value.d.token.range);
                                fadedRanges.push(x.expanders.value.d.token.range);
                            }
                            if (feature.validRadii.value.u.value === 0) {
            ...
            [...file.results.bases].forEach((x) => {
                let colorRanges = {};
                const half = Math.floor(x.value.data.value.matrix.length / 2) + 1;

                const offsets = findOffsets(half, half, x.value.data.value);

                // let upRange, downRange, rightRange, leftRange;
                // Logger.out(x.colors.);
                x.value.data.value.matrix.forEach((r, yindex) => {
                    r.value.forEach((c, xindex) => {
                        allRanges.push(c.value.label.token.range);
                        ...
                        if (yindex + 1 === offsets.centery && xindex + 1 > offsets.centerx && xindex + 1 < offsets.centerx + offsets.side) {
                            radiiRanges.push(c.value.label.token.range);
                        } else if (
                        ...
                        if (c.value.type.value === 'color' || c.value.type.value === 'filter') {
                            const color = x.value.colors.value[c.value.label.value].value.rgba.value;
                            if (!colorRanges[color]) {
                                colorRanges[color] = [];
                            }
                            colorRanges[color].push(c.value.type.token.range);
                ...
                const dec = vscode.window.createTextEditorDecorationType({
                    dark: {
                        after: {
                            contentText: ` (${x.value.data.value.matrix[0].value.length}, ${x.value.data.value.matrix.length})`,
                            fontStyle: 'bold',
                            color: 'rgba(5,6,255,0.4)',
                        },
                    },
                ...
                Decorator.colorDecorators.push(dec);
                Helper.editor.setDecorations(dec, [file.document.lineAt(x.value.data.token.range.start.line)]);
